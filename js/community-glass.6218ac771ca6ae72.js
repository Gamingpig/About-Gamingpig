...
function installGlassAdmin(){
 ...
 const host=document.getElementById('admin-dashboard');if(!host)return;const panel=document.createElement('section');panel.id='gp-glass-admin';panel.className='glass-card';panel.hidden=true;
...
const bootSharedGlass=async()=>{installOptics();await loadGlassConfig();installUniversalFooter();installGlassAdmin();};
